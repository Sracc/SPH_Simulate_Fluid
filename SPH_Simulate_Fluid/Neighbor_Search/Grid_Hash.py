...
class HashGrid:
    def __init__(self, particle_system):
        ...
        # grid table
        grid_snode = ti.root.pointer(ti.i, self.ps.particle_num[None]*2)
        grid_snode.place(self.grid_particles_num)
        # Maximum Neighbor 100
        grid_snode.dense(ti.j, 100).place(self.grid2particles)
        # neighbor table
    ...
    @ti.func
    def get_cell_index(self, gird_index):

        p1 = 73856093 * gird_index.x
        p2 = 19349663 * gird_index.y
        p3 = 83492791 * gird_index.z
        # s=int((p1 ^ p2 ^ p3) % (self.ps.particle_num[None] * 2))
        # return [(p1 ^ p2 ^ p3) % (self.ps.particle_num[None] * 2),0,0]
        return (p1 ^ p2 ^ p3) % (self.ps.particle_num[None] * 2)
        # return (pos / (self.ps.dh*2)).cast(int)
    ...
    def update_hash_table(self):
        ...
        for p_i in range(self.ps.particle_num[None]):
            ...
            gird_index = (self.ps.positions[p_i] / (self.ps.dh * 2)).cast(int)
            grid_cell = self.get_cell_index(gird_index)
    ...
    def search_neighbors(self):
        for p_i in range(self.ps.particle_num[None]):
            ...
            nb_i = 0
            cell = (self.ps.positions[p_i] / (self.ps.dh * 2)).cast(int)
            for offs in ti.static(
                    ti.grouped(ti.ndrange(*((-1, 2),) * 3))):
                range_cell = cell + offs
                if self.validate_grid(range_cell) == 1:
                    grid_cell = self.get_cell_index(range_cell)
                    for j in range(self.grid_particles_num[grid_cell]):
                        p_j = self.grid2particles[grid_cell, j]
